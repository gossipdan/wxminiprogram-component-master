...
Component({
  ...
  properties: {
    ratio: {
      ...
      observer: function (newVal, oldVal) {
        this.setData({
          width: device.windowWidth * 0.81,
          height: device.windowWidth * 0.81 / newVal
        })
      }
    },
    url: {
   ...
   */
  data: {
    width: device.windowWidth * 0.81,                //剪裁框的宽度
    height: device.windowWidth * 0.81,               //剪裁框的长度
  ...
  methods: {
    ...
    confirmCropper:function() {
      ...
      // console.log("dheight:", _this.data.originImg.height * cropData.scale)
      ctx.restore();
      ctx.draw(false, () => {
    ...
    // 完成裁剪 传递 url 路径给父组件
    sendCropperImg: function (response){
      this.triggerEvent("crop", { url: response.tempFilePath })
    },
